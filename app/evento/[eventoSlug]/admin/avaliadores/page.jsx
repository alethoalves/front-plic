...
const Page = ({ params }) => {
  ...
  const paginatorRight = (
    <Button type="button" icon="pi pi-download" text onClick={() => {}} />
  );
  ...
  const renderModalContent = () => (
    <Modal isOpen={isModalOpen} onClose={closeModalAndResetData}>
      <div className="mb-2">
        <h4>{!avaliadorToEdit ? "Incluir" : "Editar"} Avaliador</h4>
        <p>
          Preencha os dados abaixo para{" "}
          {!avaliadorToEdit ? "incluir" : "editar"} o avaliador.
        </p>
        {!avaliadorToEdit && (
          <CPFVerificationForm
  ...
  const atualizarAvaliadores = async (
    ...
    try {
      const data = await consultarAvaliadoresEvento(params.eventoSlug);
      console.log(data);
      setAvaliadores(data);
