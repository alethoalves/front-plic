...
import { RiArrowLeftLine, RiArrowRightLine, RiLogoutBoxRLine } from "@remixicon/react";
...
import {
  getAnaliseRecursoAvaliador,
  analisarRecursosEmLoteAvaliador,
...
// e na análise de recurso do gestor — switch pelo tipo de campo do formulário de inscrição.
...
const contarPendentes = (fichas) =>
  fichas.flatMap((f) => f.itens).filter((i) => i.podeEditar).length;
...
const ItemRecurso = ({ item, nota, comentario, onChangeNota, onChangeComentario }) => {
  const podeEditar = item.podeEditar;
  ...
  return (
    <div className={styles.itemRecurso}>
      ...
      <p className={styles.recursoOrientadorLabel}>Justificativa do recurso</p>
      <p className={styles.recursoOrientadorTexto}>{item.justificativaOrientador}</p>
        ...
        item.peso === null ? (
          ...
          <>
            <div className={styles.sliderBloco}>
              ...
              <Button className="btn-secondary" onClick={() => onChangeNota(item.peso)}>
              ...
              <>
                ...
                {comentario.trim().length === 0 && (
                  <p className={styles.avisoComentario}>
                    Comentário obrigatório quando a nota atribuída não é a máxima.
                  </p>
                )}
              </>
            )}
...
const Page = ({ params }) => {
  ...
  const fetchAnalise = async () => {
    setLoading(true);
    try {
      const data = await getAnaliseRecursoAvaliador(params.tenant, params.planoId);
      setAnalise(data);
    } catch (error) {
      const detail =
      ...
      router.replace(`/${params.tenant}/avaliador/avaliacoes/recursos`);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Preenche o rascunho local com os itens ainda editáveis (todas as abas),
  // sem sobrescrever o que o avaliador já estiver editando.
  useEffect(() => {
    if (!analise) return;
    setDecisoes((prev) => {
      const next = { ...prev };
      ...
      for (const ficha of todasFichas) {
        for (const item of ficha.itens) {
          if (item.podeEditar && !(item.recursoId in next)) {
            next[item.recursoId] = {
  ...
  const handleChangeNota = (recursoId, nota) =>
    setDecisoes((prev) => ({ ...prev, [recursoId]: { ...prev[recursoId], nota } }));
  const handleChangeComentario = (recursoId, comentario) =>
    setDecisoes((prev) => ({ ...prev, [recursoId]: { ...prev[recursoId], comentario } }));

  // Itens editáveis cobrem TODAS as abas — o envio em lote é global, tudo-ou-
  // nada pra família inteira reivindicada por este avaliador.
  ...
  const itensPendentes = useMemo(
    () => todosItens.filter((i) => i.podeEditar),
    [todosItens],
  );
  ...
  const handleSalvarLote = async () => {
    ...
    try {
      ...
      const resultado = await analisarRecursosEmLoteAvaliador(params.tenant, { itens });
      const atualizadosPorId = new Map(resultado.recursos.map((r) => [r.id, r]));
      setAnalise((prev) => {
        if (!prev) return prev;
        return {
          ...prev,
          projeto: prev.projeto
            ? { ...prev.projeto, fichas: atualizarFichasComResultado(prev.projeto.fichas, atualizadosPorId) }
            : null,
  ...
  const handleDevolver = async () => {
    ...
    setErroDevolver("");
    try {
      await devolverFamiliaRecursoAvaliador(params.tenant, params.planoId);
      router.push(`/${params.tenant}/avaliador/avaliacoes/recursos`);
    } catch (error) {
      setErroDevolver(
  ...
  return (
    <div>
      ...
      <ModalDelete
        ...
        confirmationText="Tem certeza que deseja devolver a análise deste recurso? Os itens ainda não decididos voltam para a fila e ficam disponíveis para outro avaliador."
      ...
      >
        <RiArrowLeftLine />
        Voltar para a Sala de Recursos
      </Link>
        ...
        <>
          {analise.temProjeto && (
            <div className={styles.abas}>
              ...
              >
                Projeto
                {pendentesProjeto > 0 && (
                  <span className={styles.abaNota}>{pendentesProjeto} pend.</span>
                )}
              </button>
              {analise.planos.map((p, i) => {
                ...
                return (
                  ...
                  >
                    Plano {i + 1}
                    {pendentesPlano > 0 && (
                      <span className={styles.abaNota}>{pendentesPlano} pend.</span>
                    )}
                  </button>
                );
              })}
          ...
          <div className={styles.navContent}>
            <div className={styles.projeto}>
              <span className={styles.eyebrow}>Análise de recurso — {labelNota}</span>
              <h4 className={styles.titulo}>{stepAtivo.titulo}</h4>
              {stepAtivo.area?.area && <p className={styles.area}>{stepAtivo.area.area}</p>}

              {(stepAtivo.conteudo?.length > 0 || stepAtivo.cronograma?.length > 0) && (
                <>
                  <h6 className={styles.secaoTitulo}>Conteúdo do {labelNota}</h6>
                  <Accordion>
                    ...
                    {stepAtivo.cronograma?.length > 0 && (
                      <AccordionTab header="Cronograma">
                        <Timeline
                          ...
                          opposite={(item) => <small>{`${item.inicio} – ${item.fim}`}</small>}
                      ...
                      <div className={styles.irParaPlanoBotoes}>
                        {analise.planos.map((p, i) => (
                          ...
                          >
                            ...
                            {analise.planos.length > 1 ? `Plano ${i + 1}` : "plano"}{" "}
                            clicando aqui
                          </Button>
                        ))}
                      </div>
                ...
                stepAtivo.fichas.map((ficha) => (
                  <div key={ficha.id} className={styles.blocoAvaliador}>
                    <h6 className={styles.secaoTitulo}>
                      Avaliação {ficha.numero} ({ficha.notaTotal}/{ficha.notaMaxima})
                    </h6>
                    {ficha.itens.map((item) => (
                      <ItemRecurso
                        ...
                        comentario={
                          decisoes[item.recursoId]?.comentario ?? item.comentarioGestor ?? ""
                        }
                        onChangeNota={(nota) => handleChangeNota(item.recursoId, nota)}
          ...
          <div className={styles.acoes}>
            {itensPendentes.length === 0 ? (
              <p className={styles.avisoManual}>Todos os recursos já foram analisados.</p>
            ) : (
              <>
                <Button
                  className="btn-secondary"
