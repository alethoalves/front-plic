...
import styles from "./ComprovanteInscricao.module.scss";
import {
  RiAwardLine,
  RiBookOpenLine,
  RiCalendarLine,
...
// ─── Component ────────────────────────────────────────────────────────────────
const ComprovanteInscricao = ({ tenant, idInscricao }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getInscricaoUserById(tenant, idInscricao);
        setInscricao(data);
      } catch {
        setNotFound(true);
      } finally {
