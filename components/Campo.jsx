...
import { RiCloseLargeLine, RiCloseLine, RiEditLine, RiEyeLine, RiSave2Line } from '@remixicon/react';
...
//FUNÇÕES
import { createCampo, createResposta, deleteFile, updateCampo, updateResposta, uploadFile } from '@/app/api/clientReq';
import Link from 'next/link';

const Campo = ({ tenantSlug, camposForm, schema, respostas, registroAtividadeId, onClose, onSuccess }) => {
  //ESTADOS
  ...
  const respostaSchema = useMemo(() => {
    ...
    if (schema?.maxChar) {
      baseSchema = baseSchema.max(schema.maxChar, { message: `Limite de ${schema.maxChar} caracteres excedido` });
    }
    ...
    switch (schema?.tipo) {
      case 'email':
        ...
        break;
      case 'number':
        baseSchema = z.number({ message: 'Número inválido' }).refine(val => !isNaN(val), { message: 'Número inválido' });
        break;
        case 'arquivo':
          baseSchema = z.instanceof(File, { message: 'Arquivo inválido' })
            .refine(file => {
              if (schema?.tipoFile === 'video') {
                return ['video/mp4', 'video/mov', 'video/avi'].includes(file.type);
              ...
              if (schema?.tipoFile === 'video') {
                return { message: 'Apenas arquivos .mp4, .mov e .avi são permitidos' };
              } else if (schema?.tipoFile === 'pdf') {
  ...
  const initialDataArray = respostas.filter(item => item.campoId === schema?.id);
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      setError(error.response?.data?.error?.message ?? "Erro ao fazer upload do arquivo.");
      throw error;
    } finally {
      setLoading(false);
    }
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      ...
      setError(error.response?.data?.error?.message ?? error.response?.data?.error  ?? "Erro na conexão com o servidor.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
    {schema?.tipo === 'arquivo' &&<p className={styles.labelFile}>{schema?.label}</p>}
    <form className={styles.formulario} onSubmit={handleSubmit(handleFormSubmit)}>
      ...
      <div className={styles.input}>
        ...
        {schema?.tipo === 'select' ? (
          ...
          <div>
            ...
            <Link prefetch={false}  href={initialData.value}  target="_blank" rel="noopener noreferrer">
          ...
          {(editar || schema?.tipo !== 'arquivo' || !initialData) && (
            <Input
              ...
              disabled={loading || (!editar && initialData && schema?.tipo !== 'arquivo')}
            />
          )}
      ...
      <div className={styles.actions}>
        ...
        {(!initialData && !editar) || (editar || !initialData) &&schema?.tipo === 'arquivo' && (
          <Button
