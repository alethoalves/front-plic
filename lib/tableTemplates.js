...
export const notaRowFilterTemplate = (options) => {
  ...
  // Componente estável (mesmo motivo do TextFilterEnterOnly abaixo: precisa
  // manter estado local dos dois campos, então não pode ser recriado a cada
  // render do template sob pena de perder o foco a cada tecla).
  //
  // Variante "só aplica ao confirmar" de notaRowFilterTemplate/
  // inteiroRowFilterTemplate — para tabelas com filtro/paginação
  // SERVER-SIDE, onde o InputNumber do PrimeReact chamando
  // filterApplyCallback a cada dígito significaria uma requisição por
  // caractere digitado (mesmo problema do filtro de texto, só que aqui pros
  // campos Mín/Máx). NÃO usar isso nas tabelas que filtram no navegador
  // (a maioria — SolicitacoesBolsa.jsx, TabelaPlanoDeTrabalho.jsx, etc.):
  // lá o filtro a cada tecla é o comportamento certo, sem custo de rede.
  const IntervaloNumericoFilterEnterOnly = ({ options, casasDecimais }) => {
    const [min, setMin] = useState(options.value?.[0] ?? null);
    const [max, setMax] = useState(options.value?.[1] ?? null);

    useEffect(() => {
      setMin(options.value?.[0] ?? null);
      setMax(options.value?.[1] ?? null);
    }, [options.value]);

    const apply = () => options.filterApplyCallback([min, max]);
    const onKeyDown = (e) => {
    ...
    return (
      <div className="flex gap-1" style={{ alignItems: "center", gap: "8px" }}>
        ...
        <InputNumber
          ...
          style={{ minWidth: "60px" }}
        />
        <Button icon="pi pi-search" onClick={apply} tooltip="Buscar" />
      </div>
    );
  };

  export const notaRowFilterEnterOnlyTemplate = (options) => (
    <IntervaloNumericoFilterEnterOnly options={options} casasDecimais={2} />
  );

  export const inteiroRowFilterEnterOnlyTemplate = (options) => (
    <IntervaloNumericoFilterEnterOnly options={options} casasDecimais={0} />
  );
