...
export const calcularFracao = (escala, valor) => {
...
/**
 * Caminho inverso de `flattenRespostas`: parte de uma árvore já respondida
 * (como a salva em FichaAvaliacao.respostas) e devolve o Map<criterioId,
 * {valorSelecionado, comentario}> que o formulário usa como estado — usado
 * pra pré-popular o formulário ao "editar avaliação" com os valores antigos.
 */
export const arvoreParaValores = (nos = [], acc = new Map()) => {
  for (const no of nos) {
    if (no.tipo === "grupo") arvoreParaValores(no.itens, acc);
    else if (typeof no.valorSelecionado === "number")
      acc.set(no.id, { valorSelecionado: no.valorSelecionado, comentario: no.comentario });
  }
  return acc;
};
