...
/**
 * Calcula pontosObtidos/pontosMaximos em toda a árvore a partir de um Map<criterioId, valor>.
 * Não faz mutação — devolve uma nova árvore anotada, pronta tanto para exibir subtotais ao
 * vivo quanto para montar o payload de envio (`flattenRespostas`).
 */
export const calcularArvoreComNotas = (nos = [], valoresPorId) => {
  ...
  const arvore = nos.map((no) => {
    ...
    const valorSelecionado = valoresPorId.get(no.id);
    ...
    return { ...no, valorSelecionado, respondido, pontosObtidos: round2(pontos) };
  });
  return { arvore, pontosObtidos: round2(pontosObtidos), pontosMaximos: round2(pontosMaximos) };
};

/** Achata a árvore calculada em [{id, valorSelecionado}] — payload que a API espera. */
export const flattenRespostas = (nosCalculados = [], acc = []) => {
  for (const no of nosCalculados) {
    ...
    else if (no.respondido) acc.push({ id: no.id, valorSelecionado: no.valorSelecionado });
  }
  return acc;
};
