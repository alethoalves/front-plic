...
export  const getAuthHeadersClient = () => {
  ...
  // Mesma decodificação de getCurrentUserId, mas lendo o claim "nome" —
  // usado quando é preciso exibir/enviar o nome do usuário logado sem uma
  // chamada extra à API (o token já carrega esse claim, ver createToken em
  // api-plic/src/services/servicesAuth.js).
  export const getCurrentUserNome = () => {
    try {
      const token = getAuthToken();
      if (!token) return null;
      const payload = JSON.parse(atob(token.split(".")[1]));
