...
export const renderDynamicFields = (formulario, control, loading, register, errors, watch, handleFieldChangeWithRules, options = {}) => {
    if (!formulario || !formulario.campos) return null;

    const { skipRequired = false, tenantSlug, respostaIdByCampoId = {} } = options;
    const campos = skipRequired
      ? formulario.campos.map((campo) => ({ ...campo, obrigatorio: false }))
      : formulario.campos;
        ...
        case "arquivo": // Campo de arquivo
          ...
          return (
            <FileInputForDynamics
              ...
              respostaId={respostaIdByCampoId[campo.id]}
              handleOnChange={(e) => {
